...
class User:
    ...
    def generate_id(self):
        ...
        try:
            self._db_user.cursor.execute(sql_query)
            row_count = self._db_user.cursor.fetchone()[0]  # fetchone returns a tuple
    ...
    def save_user_data(self):
        ...
        try:
           self._db_user.cursor.execute(sql_query,sql_data)
           self._db_user.connection.commit()
           return True
        except Exception as e:
            print(f"Error executing query: {e}") 
            if self._db_user.connection.is_connected():
                self._db_user.connection.rollback()
            return False
    ...
    def verify_user(self):
        ...
        try:
            self._db_user.cursor.execute(sql_query, sql_data)
            user_record = self._db_user.cursor.fetchone()
            if user_record is None:
                return False

            stored_encrypted_password = user_record[3]
            if(self.encrypt_password() == stored_encrypted_password):
                self.set_user_id()
                return True
            else:
                return False
